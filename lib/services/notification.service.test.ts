...
test('send formats the SMS message correctly', () => {
  ...
  expect(log.message).toMatch(/SMS.*\+60123456789.*locker-1.*A3F9C1/)
})
...
test('multiple sends append to the log', () => {
  ...
  svc.send('+601', 'L1', 'AAA111')
  svc.send('+602', 'L2', 'BBB222')
